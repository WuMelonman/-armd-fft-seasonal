...
class TrendConvNet(nn.Module):
    ...
    def __init__(self, feature_size: int, kernel_size: int = 25):
        super().__init__()
...
def train_trend_conv(model: TrendConvNet, dataloader, device, epochs: int = 3, kernel_size: int = 25, lr: float = 1e-3):
    model.train()
...
def plot_trend_decomposition(loader, trend_conv: TrendConvNet, kernel_size: int, seasonal_period: int = 24, save_path: str = 'trend_decomposition.png'):
    ...
    axes[2].plot(t, stl.seasonal)
    axes[2].set_title('Seasonal (STL)')
    axes[3].plot(t, stl.resid)
